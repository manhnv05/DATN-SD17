...
function ProductDetailInfoModal(props) {
    ...
    useEffect(() => {
        if (props.open && props.detail?.id) {
            fetch(apiUrl(`/hinhAnh/by-product-detail/${props.detail.id}`))
                .then(res => res.json())
                .then(imgList => {
                    const imgUrls = Array.isArray(imgList)
                        ? imgList.filter(img => img.duongDanAnh).map(img => img.duongDanAnh)
                        : [];
                    setImages(imgUrls);
                })
                .catch(() => setImages([]));
    ...
    useEffect(() => {
        if (props.open && props.detail && !didShowToast.current) {
            ...
            //if (!images || images.length === 0) missingFields.push("Hình ảnh");
                        ...
                        <Grid item xs={12} md={5}>
                            <Box sx={{
                                ...
                                <Typography fontSize={13} color="#888" mt={2}>
                                    Ảnh đầu tiên là ảnh đại diện của sản phẩm chi tiết.
                                </Typography>
                            </Box>
                        </Grid>
                    </Grid>
