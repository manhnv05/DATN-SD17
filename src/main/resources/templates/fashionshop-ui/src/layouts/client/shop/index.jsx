...
export default function ShopPage() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8080/mauSac/all")
      .then((res) => setColorOptions(res.data))
      .catch(() => setColorOptions([]));
    axios
      .get("http://localhost:8080/kichThuoc/all")
      .then((res) => setSizeOptions(res.data))
      .catch(() => setSizeOptions([]));
    axios
      .get("http://localhost:8080/thuongHieu/all")
      .then((res) => setBrandOptions(res.data))
      .catch(() => setBrandOptions([]));
    axios
      .get("http://localhost:8080/danhMuc/all")
      .then((res) => setCategoryOptions(res.data))
      .catch(() => setCategoryOptions([]));
    axios
      .get("http://localhost:8080/api/shop/max-price")
      .then((res) => {
  ...
  const fetchProducts = () => {
    ...
    axios
      .get("http://localhost:8080/api/shop/products", { params })
      .then((res) => {
  ...
  const handleColorClick = (color) => {
    setSelectedColors((prev) => (prev.includes(color) ? prev.filter((c) => c !== color) : [color]));
    setPage(0);
  };

  const handleSizeChange = (e) => {
    const { value, checked } = e.target;
    setSelectedSizes((prev) => (checked ? [value] : []));
    setPage(0);
  };

  const handleBrandChange = (e) => {
    const { value, checked } = e.target;
    setSelectedBrands((prev) => (checked ? [value] : []));
    setPage(0);
  };

  const handleCategoryChange = (e) => {
    const { value, checked } = e.target;
    setSelectedCategories((prev) => (checked ? [value] : []));
    setPage(0);
  };
        ...
        >
          <FilterBlock>
            ...
            <Stack direction="row" alignItems="center" gap={1.2} mb={2}>
              ...
              <Typography
                sx={{ fontWeight: 900, fontSize: 20, color: "#205072", letterSpacing: 0.5 }}
              >
                Bộ lọc
              </Typography>
              <Button
            ...
            />
            <Typography sx={{ fontWeight: 600, fontSize: 16, mb: 0.8, color: "#205072" }}>
              Danh mục
            </Typography>
            <FormGroup>
            ...
            <Typography sx={{ fontWeight: 600, fontSize: 16, mt: 2, mb: 0.8, color: "#205072" }}>
              Giá
            </Typography>
            <Box px={1.5} pb={1.5}>
              <Slider
              ...
              {colorOptions.map((color) => (
                <IconButton
                  ...
                  sx={{
                    ...
                    width: 28,
                    height: 28,
                    transition: "border 0.15s, transform 0.15s", // Thêm transform vào transition
                    ...
                    // THÊM ĐOẠN NÀY VÀO
                    "&:hover": {
                      bgcolor: color.maMau, // Giữ nguyên màu nền khi hover
                      transform: "scale(1.1)", // (Tùy chọn) Thêm hiệu ứng phóng to nhẹ để có phản hồi
                    },
                  }}
                />
              ))}
                ...
                products.map((item, idx) => (
                  <Grid item xs={12} sm={6} md={4} lg={3} key={item.id}>
                    ...
                    >
                      ...
                      <Typography
                        fontWeight={800}
                        sx={{ fontSize: 17, mb: 0.5, color: "#205072", letterSpacing: 0.3 }}
                      >
                        {item.name}
                      </Typography>
                      <Rating
                      ...
                      <Box sx={{ mt: "auto", width: "100%" }}>
                        ...
                        >
                          <Tooltip
                            title={favoriteIndexes.includes(idx) ? "Bỏ yêu thích" : "Yêu thích"}
                          >
                            <IconButton
                              sx={{
                                color: favoriteIndexes.includes(idx) ? "#e53935" : "#bbb",
                                border: favoriteIndexes.includes(idx)
                                  ? "2px solid #e53935"
                                  : "2px solid #ececec",
                                bgcolor: "#fff",
                                borderRadius: "50%",
                                boxShadow: favoriteIndexes.includes(idx)
                                  ? "0 4px 16px #ffe6e6"
                                  : "none",
                                "&:hover": {
                                  color: "#e53935",
