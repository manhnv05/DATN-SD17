...
const CheckoutPage = () => {
  ...
  const getEstimatedShippingFee = (to_district_id) => {
    ...
    const feeRanges = {
      hanoi_inner: 25000, hanoi_outer: 35000, major_cities: 45000, other_provinces: 55000,
    };
...
const getShippingFeeFromGHN = async (shippingInfo) => {
    ...
    for (const altDistrict of alternativeDistricts) {
      try {
        ...
        const serviceResponse = await ghnApi.get("/v2/shipping-order/available-services", {
          params: { shop_id: GHN_SHOP_ID, from_district: altDistrict.district_id, to_district: to_district_id },
        });
        ...
        const errorMessage = error.response?.data?.code_message_value || error.response?.data?.message || error.message;
  ...
  const handlePlaceOrder = () => {
    // Validation
    if (!formData.fullName || !formData.phone || !formData.address || !selectedProvince || !selectedDistrict || !selectedWard) {
  ...
  const discount = 0; // Logic giảm giá có thể thêm ở đây
  const orderTotal = itemSubtotal + shippingFee - discount;
            ...
            <Paper sx={{ p: { xs: 2, md: 3 }, borderRadius: 2 }}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  <TextField fullWidth label="Họ và tên" name="fullName" value={formData.fullName} onChange={handleFormChange} required />
                </Grid>
                <Grid item xs={12} sm={6}>
                  <TextField fullWidth label="Số điện thoại" name="phone" value={formData.phone} onChange={handleFormChange} required />
                </Grid>
                ...
                <Grid item xs={12}>
                  <TextField fullWidth label="Địa chỉ cụ thể" placeholder="Số nhà, tên tòa nhà, tên đường,..." name="address" value={formData.address} onChange={handleFormChange} required />
                </Grid>
                <Grid item xs={12}>
                  <TextField fullWidth multiline rows={2} label="Ghi chú" name="note" value={formData.note} onChange={handleFormChange} />
                </Grid>
              </Grid>
<Typography variant="h6" fontWeight={600} mt={4} mb={2}>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                ...
                >
                  ...
                  <LocalAtmIcon sx={{ color: 'orange',width: 50, height: 50, objectFit: 'contain', mx: 1}} />
            ...
            <Paper sx={{ p: { xs: 2, md: 3 }, borderRadius: 2, position: "sticky", top: 100 }}>
                    ...
                    cartItems.map((item) => (
                      <Stack direction="row" key={item.id} spacing={2} alignItems="center">
                        ...
                        <Badge badgeContent={item.quantity} color="primary">
                          <Avatar src={item.image} variant="rounded" sx={{ width: 64, height: 64, border: "1px solid #ddd" }} />
                        </Badge>
                        <Box flexGrow={1}>
                          <Typography variant="body2" fontWeight={500}>{item.name}</Typography>
                          <Typography variant="caption" color="text.secondary">Size: {item.size}</Typography>
                        </Box>
                        <Typography variant="body2" fontWeight={500}>{formatCurrency(item.price)}</Typography>
                      </Stack>
                    ))
                  ) : (
              ...
              <Stack spacing={1.5}>
                ...
                <Stack direction="row" justifyContent="space-between">
                  ...
                  <Typography fontWeight={500}>- {formatCurrency(discount)}</Typography>
                </Stack>
              </Stack>
              ...
              <Button fullWidth variant="contained" color="error" size="large" sx={{ py: 1.5, fontWeight: 700 }} onClick={handlePlaceOrder} disabled={isCalculatingFee || shippingFee === 0 && selectedWard !== null}>
