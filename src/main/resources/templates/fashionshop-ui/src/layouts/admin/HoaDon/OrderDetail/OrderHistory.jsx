...
const OrderHistory = ({ orderId ,onOrderUpdate}) => {
  ...
  const formatDateTime = useCallback((isoString) => {
    ...
    return `${pad(formattedHours)}:${pad(minutes)}:${pad(seconds)} ${ampm} ${pad(day)}/${pad(
      month
    )}/${year}`;
  }, []);

  const mapStatusToDisplay = useCallback(
    (apiStatus, apiDate) => {
      let displayStatus = "";
      let className = "";
      const trimmedStatus = apiStatus ? apiStatus.trim() : "";
      switch (
        trimmedStatus // Sử dụng biến đã được trim
      ) {
        case "Tạo đơn hàng":
  ...
  const getIconComponent = useCallback((className) => {
    ...
    switch (className) {
      ...
      case "DA_XAC_NHAN":
        return <img src={da_xac_nhan} alt="Đã xác nhận" style={imgStyle} />;
      case "CHO_XAC_NHAN":
      ...
      case "HUY":
        return <img src={Huy} alt="Hoàn thành" style={imgStyle} />;
      default:
        return null;
    }
  }, []);
  ...
  // === BẮT ĐẦU PHẦN THÊM MỚI: LẮNG NGHE WEBSOCKET ===
  useEffect(() => {
    if (!orderId) return;
    ...
    const client = new Client({
      ...
      onConnect: () => {
        ...
        client.subscribe("/topic/admin/order-updates", (message) => {
          ...
          // 1. Kiểm tra ID nhận được và ID hiện tại
          console.log(
            `Message ID: ${updatedOrder.idHoaDon} (type: ${typeof updatedOrder.idHoaDon})`
          );
    ...
    return () => {
      console.log(`Deactivating WebSocket for Order ID: ${orderId}`);
      client.deactivate();
    };
  }, [orderId, fetchOrderHistory,onOrderUpdate]);
  ...
  if (historyData.length === 0)
    return <Typography sx={{ textAlign: "center", p: 3 }}>Chưa có lịch sử trạng thái.</Typography>;
...
OrderHistory.propTypes = {
  ...
  onOrderUpdate: PropTypes.func,// Giả sử orderId là một chuỗi và là bắt buộc
};
